...
function DetailsPage(props) {
    ...
    const fetchDetailsData = () => {
        axios.get(`https://openlibrary.org${props.location.state.selectedKey}.json`)
        .then((response)=>{
    ...
    return (
        ...
        <h1 className='mb-5 mt-5'><span>Book Name: </span>{detailsData.title}</h1>
        ...
        <h1 className='mt-5'>Published Year: <span>{detailsData.first_publish_date && detailsData.first_publish_date}</span> </h1>
        <h1 className='mt-5'>Description: </h1>
        <p className='h2'>{detailsData.description&&detailsData.description.value}</p>
